...
!   - compute larva mortality (T > 42 for only one time step)
  ...
  do k=1,nveg   ! Boucle sur les voxels occups par une feuille mine
    do je=1,nje(k)
     jent=nume(je,k)
     if (ismine(jent).eq.1) then  ! Le voxel contient des feuilles mines
...
!  Computing developmental rate (dev_rate, h-1): Boucle sur les voxels occups par une feuille mine: Eqn 4 SP et al. 2007
