...
! OTHER CONSTANT PARAMETERS
 ...
 subroutine Farquhar_scaling_factors
  ...
  real :: VCmax25  ! maximum carboxylation rate at 25C
  real :: Jmax25  ! maximum electron transfert rate at 25C
  real :: Rd25   ! dark respiration rate at 25C
...
!  Assimilation rate computation from Farquhar's model applied to shaded/sunlit area of each vegetation type in each voxel
  ...
  do k=1,nveg
   do je=1,nje(k)
    jent=nume(je,k)
    do joe=0,1
     rco2Pa=rco2(joe,je,k)*101325. ! Conversion into mol CO2-1 m2 s Pa
     A=0.
     call Farquhar_model_1(A,crespd(je,k),cvcmax(je,k),cjmax(je,k),rco2Pa,ts(joe,je,k)+273.15,PARirrad(joe,je,k),caref)

     A = A * S_detailed(joe,je,k)  ! Net A rate in mol CO2 s-1
...
!  Normalisation of A_rates by leaf area
 ...
 real :: aa, bb, ccc, dd
  ...
  respd=exp(crespd0-(dharespd/r/leaf_tempK)) ! Dark respiration rate (mol CO2 m-2 s-1)
  vcmax=exp(cvcmax0-(dhavcmax/r/leaf_tempK))/(1+exp(dsvcmax/r- (dhdvcmax/r/leaf_tempK))) ! Maximum carboxylation rate VCmax (mol CO2 m-2 s-1)
  zjmax=exp(cjmax0-(dhajmax/r/leaf_tempK))/(1+exp(dsjmax/r- (dhdjmax/r/leaf_tempK)))  ! Maximum electron transfert rate Jmax (mol e m-2 s-1)

!  Actual electron flux J (mol e m-2 s-1)
  aa=alpha*leaf_PAR
  zj=aa/sqrt(1+(aa/zjmax)**2)

!  Assimilation rate limited by electron flux,  WJ (mol CO2 m-2 s-1)
  aa=rco2Pa
...
!  Assimilation rate limited by Rubisco,  Wc (mol CO2 m-2 s-1)
  aa=rco2Pa
