...
"""
import numpy
from openalea.ratp.interface.clumping_index import clark_evans, expand_point, \
...
def test_expand_point():
    ...
    expected = [(0.5, 1.0, 0.0),
                ...
                (1.0, 0.5, 0.0),
                (1.0, 0.5, 0.0)]
    expanded = list(zip(*expand_point(x, y, z, area, normal, domain=domain)))
    numpy.testing.assert_array_equal(expanded, expected)

    # rotated expansion
    x1, y1, z1 = 0, 1.5, 1.5
    normal1 = (1, 0, 0)
    expanded = list(zip(*expand_point(x1, y1, z1, area, normal1)))
    expected1 = [(0, 2.5, 2.5),
                 (0, 2.5, 1.5),
                 (0, 2.5, 0.5),
                 (0, 1.5, 2.5),
    ...
    expanded = list(zip(*expand_points(xn, yn, zn, arean, normaln)))
